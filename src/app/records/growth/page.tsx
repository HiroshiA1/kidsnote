      ...
      <main className="max-w-4xl mx-auto px-6 py-6 space-y-6">
          ...
          <div className="space-y-3">
            {savedMessages.map(message => {
              ...
              return (
                <div
                  key={message.id}
                  className="bg-tertiary/10 border-l-4 border-tertiary rounded-lg p-4 hover:shadow-md transition-shadow"
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="font-medium text-headline">{data.child_names.join('、')}</div>
                      <p className="text-sm text-paragraph mt-1">{data.summary}</p>
                      {data.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {data.tags.map((tag, i) => (
                            <span key={i} className="text-xs px-2 py-0.5 bg-tertiary/30 rounded-full">
                              {tag}
                            </span>
                          ))}
                        </div>
        ...
        {savedMessages.length === 0 && pendingMessages.length === 0 && (
          <div className="text-center py-20">
            ...
            <h2 className="text-lg font-medium text-headline mb-2">成長記録がありません</h2>
