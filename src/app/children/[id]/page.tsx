...
export default function ChildDetailPage() {
  ...
  // 退園済み園児は AI の対象コンテキストに含めない(通常業務と切り離す)
  useEffect(() => {
    ...
    <div className="min-h-screen pb-8">
      ...
      <header className="sticky top-0 z-10 bg-surface/80 backdrop-blur-sm border-b border-secondary/20">
        <div className="max-w-6xl mx-auto px-3 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-4">
            <Link
              href={isArchived ? '/children/archived' : '/children'}
      ...
      {isArchived && (
        <div className="max-w-6xl mx-auto px-3 sm:px-6 mt-3">
          <div className="bg-alert/10 border border-alert/40 rounded-lg p-3 text-sm text-paragraph flex items-start gap-3">
            <svg className="w-5 h-5 flex-shrink-0 text-alert" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
            </svg>
            <div>
              <p className="font-medium text-headline">この園児は退園(アーカイブ)済みです</p>
              <p className="text-xs text-paragraph/70 mt-1">
                編集・文書生成・AI記録の対象外になります。復元するには
                <Link href="/children/archived" className="text-button hover:underline ml-1">アーカイブ一覧</Link>
                から操作してください。
              </p>
            </div>
          </div>
