  ...
  return (
    <div className="min-h-screen pb-8">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Google Calendar 連携 — 自分自身のスタッフのみ操作可能 */}
        {!isArchived && currentStaffId === staff.id && (
          <GoogleConnectSection
            ...
            onConnect={async () => {
              try {
                ...
                addToast({
                  type: 'error',
                  message: err instanceof Error ? err.message : 'Google 連携の開始に失敗しました',
                });
              }
            }}
            onDisconnect={async () => {
              try {
                ...
                addToast({ type: 'success', message: 'Google 連携を解除しました' });
              } catch (err) {
                addToast({
                  type: 'error',
                  message: err instanceof Error ? err.message : '連携解除に失敗しました',
                });
              }
            }}
          />
        )}
        {/* 他人のスタッフ詳細では連携状態のみ表示 (操作不可) */}
        {!isArchived && currentStaffId !== staff.id && staff.googleEmail && (
          <section className="bg-surface rounded-xl p-6 shadow-sm">
            <h3 className="text-lg font-bold text-headline mb-2">Google カレンダー連携</h3>
            <p className="text-sm text-paragraph">連携済み: {staff.googleEmail}</p>
            <p className="text-xs text-paragraph/60 mt-1">連携の解除は本人のアカウントからのみ可能です。</p>
          </section>
        )}
      </main>
    </div>
...
function GoogleConnectSection({
  ...
  const connected = !!staff.googleEmail;
  ...
  return (
    <section className="bg-surface rounded-xl p-6 shadow-sm">
      <h3 className="text-lg font-bold text-headline mb-4">Google カレンダー連携</h3>
      {connected ? (
        <div className="space-y-3">
          <div className="flex items-center gap-3 p-4 bg-tertiary/10 border border-tertiary/30 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-headline">連携済み</p>
              <p className="text-xs text-paragraph/60 truncate">{staff.googleEmail}</p>
            </div>
          </div>
          <p className="text-xs text-paragraph/60">
            kidsnote で予定を追加すると、あなたの Google カレンダーにも反映されます (Phase 3-3 で有効化予定)。
          </p>
          <button
            type="button"
            onClick={handleDisconnect}
            disabled={busy}
            className="px-4 py-2 bg-surface border border-alert/40 text-alert rounded-lg text-sm font-medium hover:bg-alert/10 transition-colors disabled:opacity-50"
          >
            {busy ? '処理中...' : '連携を解除'}
          </button>
        </div>
        ...
        <div className="space-y-3">
          <p className="text-sm text-paragraph">
            あなたの Google カレンダーに、kidsnote で追加した予定を自動的に反映できます。
          </p>
          <button
            type="button"
            onClick={handleConnect}
            disabled={busy}
            className="px-4 py-2 bg-button text-white rounded-lg text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50 inline-flex items-center gap-2"
          >
